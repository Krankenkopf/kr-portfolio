...
const SkillCards: FC<TSkillCardsProps> = () => {
  ...
  const [splashAnimationName, setSplashAnimationName] = useState('__splashPulse')
  ...
  useEffect(() => {
    let interval = 3000;
    const switchAnimation = (state: string): void => {
      const switchAnimationName = state === '__splashRotate' ? '__splashPulse' : '__splashRotate'
      interval = state === '__splashPulse' ? 800 : +Math.random().toFixed(2) * 4000 + 8000;
      inView && setSplashAnimationName(switchAnimationName)
      timeout.current = setTimeout(() => switchAnimation(switchAnimationName), interval)
    }
    timeout.current = setTimeout(() => {
      switchAnimation(splashAnimationName);
    }, interval)
  }, [inView])

  useEffectOnce(() => () => timeout.current && clearTimeout(timeout.current))
  ...
  const skillsCards = skillsData.map(s => (
    <div key={s.id} className="skills__card">
      ...
      <div className="skills__card__splashDesc"
        style={inView
          ? { animationName: splashAnimationName, animationDuration: splashAnimationName === '__splashPulse' ? '1.5s' : '200ms' }
          : { animationName: 'none' }}>
        {s.skillSplashDesc}
      </div>
